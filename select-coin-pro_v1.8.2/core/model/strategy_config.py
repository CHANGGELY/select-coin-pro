...
class StrategyConfig:
    ...
    def __post_init__(self):
        # 强制修正 hold_period 的时间单位 (Pandas 2.2+ 兼容)
        if isinstance(self.hold_period, str):
            # 确保 H 转换为 h (Pandas deprecation fix), D 保持大写
            fixed_period = self.hold_period.upper().replace('H', 'h')
            object.__setattr__(self, 'hold_period', fixed_period)
    ...
    def init(cls, index: int, file: DummyStrategy = None, **config):
        ...
        _config = dict(
            ...
            hold_period=file.hold_period.upper().replace('H', 'h'),
