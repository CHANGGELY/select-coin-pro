    ...
    "账户1": {
        ...
        "strategy_list": [
            ...
            },
            {
                ...
                "filter_list_post": [
                    ('VolumeMeanQ', 864, 'val:>0.5')
                ],
                "use_custom_func": False  # 使用系统内置因子计算、过滤函数
            },
            # === 1.2截面因子纯多策略+后置过滤(资金占比0.25)
            {
                # 策略名称。与strategy目录中的策略文件名保持一致。
                ...
                # 选币因子信息列表，用于`2_选币_单offset.py`，`3_计算多offset资金曲线.py`共用计算资金曲线
                "factor_list": [
                    ('RankBias', True, 1200, 1),  # 多头因子名（和factors文件中相同），排序方式，参数，权重。
                ],
                "filter_list": [],
                "filter_list_post": [
                    ('ZfMeanQ', 480, 'val:>0.5')
            ...
            },
            {
                ...
                # 选币因子信息列表，用于`2_选币_单offset.py`，`3_计算多offset资金曲线.py`共用计算资金曲线
                "factor_list": [
                    ('RankPct', True, 1632, 1),  # 多头因子名（和factors文件中相同），排序方式，参数，权重。
                ],
                "filter_list": [],
                "filter_list_post": [
                    ('VolumeMeanQ', 912, 'val:>0.5')
                ],
                "use_custom_func": False  # 使用系统内置因子计算、过滤函数
            },
            ...
            # ===2.1 先使用QuoteVolumeMean，再使用动量因子，资金占比为空头的0.6(总资金的0.3)
            {
                ...
                'short_select_coin_num': 0.25,  # 四种形式：整数， 小数，'long_nums', 区间选币：(0.1, 0.2), (1, 3)
                # 选币因子信息列表，用于2_选币_单offset.py，3_计算多offset资金曲线.py共用计算资金曲线
                "factor_list": [
                    ('Cci', False, 360, 1),  # 多空因子名（和factors文件中相同），排序方式，参数，权重。支持多空分离，多空选币因子不一样；
                ],
                "filter_list": [
            ...
            },
            {
                ...
                # 选币数量
                'long_select_coin_num': 0,  # 可适当减少选币数量，对策略收益影响大
                'short_select_coin_num': (0.25, 0.5),  # 四种形式：整数， 小数，'long_nums', 区间选币：(0.1, 0.2), (1, 3)
                # 选币因子信息列表，用于2_选币_单offset.py，3_计算多offset资金曲线.py共用计算资金曲线
                "factor_list": [
                    ('BiasQ', False, 600, 1),  # 多空因子名（和factors文件中相同），排序方式，参数，权重。支持多空分离，多空选币因子不一样；
                ],
                "filter_list": [
                    ('QuoteVolumeMean', 600, 'pct:<0.2', False),  # 后置过滤filter_list_post，三种形式：pct, rank, val；支持多空分离，多空过滤因子不一样；
                ],
                "filter_list_post": [
                    ('VolumeMeanQ', 600, 'val:<0.5'),  # 后置过滤filter_list_post，三种形式：pct, rank, val；支持多空分离，多空过滤因子不一样；
                ],
            },
            # ===2.2 先使用动量因子，再使用QuoteVolumeMean，资金占比为空头的0.2(总资金的0.1)
            {
                ...
                "offset_list": [6],  # 只选部分offset[1, 3, 6]；
                "hold_period": "24H",  # 小时级别可选1H到24H；也支持1D交易日级别
                "market": 'swap_swap',  # 多头支持交易现货；
                # 资金权重。程序会自动根据这个权重计算你的策略占比
                'cap_weight': 0.1 / 2,
                ...
                # 选币因子信息列表，用于2_选币_单offset.py，3_计算多offset资金曲线.py共用计算资金曲线
                "factor_list": [
                    ('QuoteVolumeMean', True, 216, 1),  # 多空因子名（和factors文件中相同），排序方式，参数，权重。支持多空分离，多空选币因子不一样；
                ],
                "filter_list": [
                    ('BiasQ', 216, 'pct:<0.2'),  # 后置过滤filter_list_post，三种形式：pct, rank, val；支持多空分离，多空过滤因子不一样；
                ],
                "filter_list_post": [
                    ('ZfMeanQ', 216, 'val:<0.5'),  # 后置过滤filter_list_post，三种形式：pct, rank, val；支持多空分离，多空过滤因子不一样；
                ],
            },
            {
                # 策略名称。与strategy目录中的策略文件名保持一致。
                ...
                # 选币因子信息列表，用于2_选币_单offset.py，3_计算多offset资金曲线.py共用计算资金曲线
                "factor_list": [
                    ('QuoteVolumeMean', True, 408, 1),  # 多空因子名（和factors文件中相同），排序方式，参数，权重。支持多空分离，多空选币因子不一样；
                ],
                "filter_list": [
                    ('Cci', 408, 'pct:<0.2'),  # 后置过滤filter_list_post，三种形式：pct, rank, val；支持多空分离，多空过滤因子不一样；
                ],
                "filter_list_post": [
                    ('VolumeMeanQ', 408, 'val:<0.5'),  # 后置过滤filter_list_post，三种形式：pct, rank, val；支持多空分离，多空过滤因子不一样；
                ],
            },
            # ===2.3 同时使用QuoteVolumeMean和动量因子过滤，全选剩下的币种，资金占比为空头的0.2(总资金的0.1)
            {
                ...
                "filter_list": [
                    ('Cci', 240, 'pct:<0.15'),  # 后置过滤filter_list_post，三种形式：pct, rank, val；支持多空分离，多空过滤因子不一样；
                    ('QuoteVolumeMean', 240, 'pct:<0.15', False),
                ],
                "filter_list_post": [
                    ('ZfMeanQ', 240, 'val:<0.5'),  # 后置过滤filter_list_post，三种形式：pct, rank, val；支持多空分离，多空过滤因子不一样；
                ],
            },
        ],
